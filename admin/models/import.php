...
class Hello_worldModelImport extends BaseDatabaseModel
{
	...
	**/
	...
	{
		if (Hello_worldHelper::checkArray($target_headers))
		{
			// make sure the file is loaded
			Hello_worldHelper::composerAutoload('phpspreadsheet');
	...
	**/
	protected function save($data,$table)
	{
		...
		if(Hello_worldHelper::checkArray($data['array']))
		{
			// get user object
			...
			if(Hello_worldHelper::checkArray($data['array']) && $user->authorise($table.'.import', 'com_hello_world') && $user->authorise('core.import', 'com_hello_world'))
			{
				// set target.
				...
				$canDo			= Hello_worldHelper::getActions($table);
				...
				foreach($data['array'] as $row)
				{
					...
					if (isset($row[$id_key]) && is_numeric($row[$id_key]) && $row[$id_key] > 0)
					{
						...
						$query
							->select('version')
							->from($db->quoteName('#__hello_world_'.$table))
					...
					if($found && $canEdit)
					{
						...
						$query->update($db->quoteName('#__hello_world_'.$table))->set($fields)->where($conditions);
						$db->setQuery($query);
						$db->execute();
					}
					elseif ($canCreate)
					{
						...
						$query
							->insert($db->quoteName('#__hello_world_'.$table))
						...
						if ($done)
						{
							...
							Hello_worldHelper::setAsset($aId,$table);
						}
					}
					else
					{
	...
	protected function getAliasesUsed($table)
	{
		...
		$columns = $db->getTableColumns('#__hello_world_'.$table);
		if(isset($columns['alias'])){
			// Create a new query object.
			$query = $db->getQuery(true);
			$query->select($db->quoteName(array('alias')));
			$query->from($db->quoteName('#__hello_world_'.$table));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
